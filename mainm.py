...
def get_heading(course_nodes, heading):
    heading = sendAndReceiveValue('h','z','z')

    if course_nodes.orientation == 'e':
        if heading > 180:
            initial_heading = heading - 360
        else:
            initial_heading = heading
    elif course_nodes.orientation == 's':
        initial_heading = heading - 90
    elif course_nodes.orientation == 'n':
        initial_heading = heading - 270
    elif course_nodes.orientation == 'w':
...
def turn(turn_direction, course_nodes):
    ...
    if turn_direction == 'l':
        # correct heading while turning left
        i = -187
        j = 187
    ...
    while (delta_heading > error_threshold) or (delta_heading < -error_threshold):
