...
# Updated by Rolly Noel January 2017
#
...
# You may get better performance (certainly fewer communication errors with the BrickPi, and less work using this version
#   of the BrickPi.py driver) if you place all your motors and sensors on the same Arduino (if possible).
# This version of BrickPi.py won't bother to poll (triggered by BrickPi.UpdateValues()) an Arduino without gear.

##################################################################################################################
# Debugging:
# - NOTE THAT DEBUGGING ERROR MESSAGES ARE TURNED OFF BY DEFAULT.  To debug, change Line 75 to True.
#
...
DEBUG = False # Remove to hide errors
...
#MSG_TYPE_SENSE           = 6 # Sense an Arduino's values (NOT SUPPORTED BY DEXTERNIND's FIRMWARE)
...
def BrickPiChangeAddress(OldAddr, NewAddr):		# WHAT IS THIS FOR ?????????????????????????????
...
def BrickPiReturnVersion(arduino):
    Array[BYTE_MSG_TYPE] = RETURN_VERSION
    BrickPiTx(arduino, 1, Array)
    res, BytesReceived, InArray = BrickPiRx(0.0200000)
    if res: return res
    if InArray[BYTE_MSG_TYPE] != MSG_TYPE_SENSE: return -1, BytesReceived, Array
    Versions[arduino] = InArray[1]
    return Versions[arduino]

def BrickPiSense():
    global txions, reTxions, rxions, rxionTOs, tooShortRxions, truncatedRxions, badCrcRxions
    
    print \
("Txions: %d, retxions: %d, rxions: %d, rxions t/o: %d, short rxions: %d, truncated rxions: %d, bad crc rxions: %d" \
      % ( txions, reTxions, rxions, rxionTOs, tooShortRxions, truncatedRxions, badCrcRxions))
    txions = 0 
    reTxions = 0
    rxions = 0
    rxionTOs = 0
    ...
    truncatedRxions = 0
    badCrcRxions = 0
    """  (NOT SUPPORTED BY DEXTERNIND's FIRMWARE)
        (Too bad as could get comm errors from the Arduino's point of view)
    if not Versions[arduino]: BrickPiReturnVersion(arduino)
    if Versions[arduino] <= 2: return -1, 0, Array
    Array[BYTE_MSG_TYPE] = MSG_TYPE_SENSE
    BrickPiTx(arduino, 1, Array)
    res, BytesReceived, InArray = BrickPiRx(0.0200000)
    if res: return res
    for j in range(len(InArray)-1):
        Array[j] = InArray[j+1]
    if InArray[BYTE_MSG_TYPE] != MSG_TYPE_SENSE: return -1, BytesReceived, Array
    displayHex (arduino, 'Sense', Array, BytesReceived - 1)
     """ 
    return 0
  

def zeroEncoders():
    while BrickPiUpdateValues():    # Get a current reading (ignore bad ones) 
        pass
    ...
    while BrickPiUpdateValues():    # Apply the offsets and get a current reading (ignore bad ones) 
        pass
...
def displayHex (device, prologString, numericArray, bytesOfBuffer):
    if device != 255: s = ("Arduino: %02x" % device) + ', '
    else: s = ''
    s += prologString
    if type(numericArray[0]) == str:
        if p_version==2:
            for i in xrange (bytesOfBuffer): s += ":%02x" % ord(numericArray[i])
        else:
            for i in range (bytesOfBuffer): s += ":%02x" % ord(numericArray[i])
    else:
        if p_version==2:
            for i in xrange (bytesOfBuffer): s += ":%02x" % numericArray[i]
        else:
            for i in range (bytesOfBuffer): s += ":%02x" % numericArray[i]
    print (s)
...
def BrickPiSetupSensors():
    ...
    global Retried
    if BrickPi.MotorEnable[0] or BrickPi.MotorEnable[1] or BrickPi.SensorType[0] or BrickPi.SensorType[1]:
        ArduinosToUse[0] = ARDUINO1
        print ("Using Arduino 1")
    if BrickPi.MotorEnable[2] or BrickPi.MotorEnable[3] or BrickPi.SensorType[2] or BrickPi.SensorType[3]:
        ArduinosToUse[1] = ARDUINO2
        print ("Using Arduino 2")
    for i in range(2):
        if not ArduinosToUse[i]: continue 	# If not using this Arduino, it doesn't have any sensors
        Retried = 0
        while Retried < 4:
        ...
        ########################
            ...
            #displayHex(BrickPi.Address[i], "Set up sensors txion", Array, tx_bytes)
            ...
            if (result or BytesReceived != 1 or Array[BYTE_MSG_TYPE] != MSG_TYPE_SENSOR_TYPE):
                ...
                #displayHex(255, "Set up sensors rxion", Array, BytesReceived)
...
def BrickPiUpdateValues():
    ...
    global Bit_Offset
    global Retried
    global reTxions, rxions, rxionTOs, tooShortRxions, truncatedRxions
	
    for i in ArduinosToUse:
        if not i: continue
        i -= 1          #Convert from Arduino address of 1 to index of 0
        Retried = 0
        while Retried < 4:
            
            #Retry Communication from here, if failed
            
            if Retried: reTxions += 1
            Array = [0] * 256
            ...
            for ii in range(2):
                ...
                if(BrickPi.EncoderOffset[port]):
                    ...
                    if Temp_Value < 0:
                        Temp_ENC_DIR = 1
                        Temp_Value *= -1
                    Temp_BitsNeeded = BitsNeeded(Temp_Value) #NOT INCLUDING SIGN BIT IN LENGTH SO DON'T OVERFLOW - FIRMWARE WILL KNOW ITS THERE AND INCRE LENGTH TO GET IT
          ...
          #######################
            ...
            BrickPiFlush() # Toss out any old Rx data. This helps to make sure everything is fresh, and for the correct "core".
            BrickPiTx(i + 1, tx_bytes, Array)
            if DEBUG and Retried and (Retried != 10): displayHex(Retried,"retxing",Array,tx_bytes)
            #displayHex(i+1,"txing update values",Array,tx_bytes)
            
			#HANDLE MESSAGE RXED ***************************************************************
            result, BytesReceived, InArray = BrickPiRx(0.008) #check timeout (was 0.007500)
            for j in range(len(InArray)):
                Array[j]=InArray[j]
            if result != -2 :
        ...
        if Retried and (Retried != 10): #if gave up after 4 retxions
            if DEBUG: print ("Retry Failed")
            return -1
        ...
        for ii in range(2):
            ...
            if Temp_EncoderVal & 0x01 :
                ...
                #print ("encoder # %d: %d" % (ii + i*2, BrickPi.Encoder[ii + i*2]))
...
def BrickPiSetup():
    ...
    if not ser.isOpen():
        return -1
    if DEBUG: print("Setup returned 0")
    return 0
...
def BrickPiTx(dest, ByteCount, OutArray):
    ...
    if not ArduinosToUse[dest-1]:
        print ("Blocking ", dest, ByteCount, OutArray)
        return		# Block useless polls of unused Arduinos
    #displayHex (dest, 'OutArray', OutArray, ByteCount)
    txions += 1
    if p_version==2:
        # print ('a',p_version)
        tx_buffer = ''
...
def BrickPiRx(timeout):
    global rxions, rxionTOs, tooShortRxions, truncatedRxions,badCrcRxions
    ...
    while( ser.inWaiting() <= 0):
        if time.time() - ot >= timeout :
            if DEBUG: print ("rxion timeout")
            rxionTOs += 1
            return -2, 0 , []

    if not ser.isOpen():
        if DEBUG: print ("serial port not open")
        return -1, 0 , []
          ...
          while ser.inWaiting():
            while ser.inWaiting():
                ...
                for i in buff_read:
                    ...
                    string_in+=chr(i)
                rx_buffer += (string_in)
            time.sleep(.001) #  is required. If the BrickPi is in interrupt listening to a configured (57600 baud) EV3 sensor, it could be about 170uS between bytes. 250uS should be safe.
        except:
            # print ("Unexpected error: ", sys.exc_info()[0])
            return -1, 0 , []
    ...
    if RxBytes < 2 :
        if DEBUG: displayHex(255,"rxion too short",rx_buffer,RxBytes)
        tooShortRxions += 1
        return -4, 0 , []

    if RxBytes < ord(rx_buffer[1])+2 :
        if DEBUG: displayHex(255,"rxion shorter than expected",rx_buffer,RxBytes)
        truncatedRxions += 1
        return -6, 0 , []

    CheckSum = 0
    #for i in rx_buffer[1:]:    #SAW 2 RXIONS TOGETHER SO CHECKSUM CALC'D ON THE BOTH OF THEM
    for i in rx_buffer[1:ord(rx_buffer[1])+2]:
        CheckSum += ord(i)
    ...
    if (CheckSum % 256) != ord(rx_buffer[0]) : #Checksum equals sum(InArray)+len(InArray)
        if DEBUG: 
            displayHex(255,"rxion checksum error",rx_buffer,ord(rx_buffer[0]))
            print ("checksum in rxion: %02x, calc'd checksum: %02x" % (ord(rx_buffer[0]), CheckSum % 256))
        badCrcRxions += 1
        return -5, 0 , []

    InBytes = RxBytes - 2

    #displayHex (255, 'InArray', InArray, InBytes)
    rxions += 1
